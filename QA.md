...
Q: what is the meaning of a Hub in websocket chat?

A:

Hub = `central goroutine` that owns all client state. Actor model pattern.

Channels come in:

- `register` — add client
- `unregister` — remove client
- `broadcast` — fan-out message to all

`Run()` is `select` loop processing these one-at-a-time in single goroutine. No mutex needed — state never shared, only communicated.
